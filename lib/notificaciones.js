...
async function enviarAvisoDonacion(datos, webhookUrl) {
  ...
  try {
    ...
    clearTimeout(timeoutId);
    /* 2.33: el guard de pureza de lib/ prohíbe escribir en la consola desde aquí,
       así que el error se devuelve en el resultado y lo registra el llamador.
       Se descarta la URL del mensaje: un webhook puede llevar token en la ruta. */
    return {
      enviado: false,
      motivo: 'error_red',
      error: String(err.name || 'Error'),
      detalle: String(err.message || '').replace(/https?:\/\/\S+/g, '[url]').slice(0, 160),
    };
  }
}
