  ...
  return function renderShell({ title, description, canonicalPath = '/', rootContent = '', jsonLd = null, vehicleId = null, nonce = '', ogImage = null, staticApp = false, keepPlaceholder = false }) {
    ...
    if (ADSENSE_CLIENT) {
      /* 2.31: el contenedor de anuncios entra como marco del mismo origen
         ...
         DÓNDE VA: solo en las páginas de contenido, arriba, que es su sitio.
         En la PORTADA de la app NO se inyecta. Medido en el celular: con el
         marco al principio de <body> aparecía una franja blanca por encima del
         logotipo, y al moverlo al final aparecía la misma franja debajo del pie
         —el cuerpo de la portada termina justo en el borde de la pantalla, así
         que cualquier bloque que se añada se ve como una banda suelta—. La
         entrada de la aplicación tiene que empezar en su encabezado y terminar
         en su pie; el bloque publicitario va donde hay contenido que leer. */
